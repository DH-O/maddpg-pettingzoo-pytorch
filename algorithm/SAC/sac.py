...
from algorithm.SAC.utils_sac import soft_update, hard_update
from algorithm.SAC.model_sac import GaussianPolicy, QNetwork, DeterministicPolicy
...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], args.hidden_size).to(device=self.device) # 액션 스페이스가 Box로 구성된 연속이었으나, 이번에는 Discrete로 구성되어있음
        self.critic_target = QNetwork(num_inputs, action_space.shape[0], args.hidden_size).to(self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.metra_lr)
        ...
        if self.policy_type == "Gaussian":
            ...
            if self.automatic_entropy_tuning is True:
                ...
                self.alpha_optim = Adam([self.log_alpha], lr=args.metra_lr)

            self.policy = GaussianPolicy(num_inputs, action_space.shape[0], args.hidden_size, action_space).to(self.device)
            self.policy_optim = Adam(self.policy.parameters(), lr=args.metra_lr)
